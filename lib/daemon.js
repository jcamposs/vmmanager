...
function boot() {
  nimble.series([
    ...
    function(callback) {
      ...
      fs.exists(Object.config.path, function (exists) {
        if (exists)
          callback();
        else {
          logger.warn("Workspace directory does not exist.");
          logger.info("Creating working directory in ", Object.config.path);
          mkdir(Object.config.path, function(err) {
            if (err)
              logger.error(err);
            else
              callback();
    ...
    function(callback) {
      ...
      fs.readdir(Object.config.path, function(err, files) {
        if (err) {
          logger.error(err);
          return;
        }
        ...
        var count = 0;
        for (var i = 0; i < files.length; i++)
          controller.load_workspace(path.join(Object.config.path, files[i]),
                                                                function(err) {
            if (err)
              logger.error(err);
