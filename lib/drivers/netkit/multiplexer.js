...
util.inherits(Multiplexer, require('events').EventEmitter);
...
console.log("CLOSING: ", this._state);
...
Multiplexer.prototype._addUser = function(socket) {
  ...
  user.on("ready", function() {
    logger.debug("User ", user.getId(), " connected to virtual machine.");
  });
...
User.prototype.configure = function() {
  ...
  obj._socket.on('data', function (buff) {
    ...
    if (obj._state == "configured")
      console.log("TODO: Send data to the virtual machine");
  });
