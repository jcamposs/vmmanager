...
Netkit.prototype.startVM = function(workspace, config, callback) {
  var netkitCfg = configureNetkitVM(workspace, config);
  logger.debug("Configuration: " + netkitCfg["cmd"]);
}

module.exports = Netkit;

function configureNetkitVM(workspace, params) {
  ...
  // Reserve a free port by using it, afterwads we will release when launching
  // the virtual machine. That's not an infallible fix due to the port can be
  // reassigned to a different process in the time it is realeased and assigned
  //again.
  //netkitCfg["svc"] = TCPServer.new 0
  netkitCfg["port"] = 9999;
  ...
  for (var i = 0; i < params["network"].length; i++) {
    ...
    if (entry["interface"] && entry["collision_domain"])
      ...
      console.log("Error: VMC protocol error");
      //config["svc"].close
      return null;
    }
  }
