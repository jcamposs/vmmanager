  ...
  nimble.series([
    ...
    function(_callback) {
      ...
      if (netkitCfg["type"] == "switch")
        createSwitchConfigFile(workspace, netkitCfg, function(err) {
          if (err)
...
function createSwitchConfigFile(workspace, config, callback) {
  var conf_script = null;

  nimble.series([
    function(_callback) {
      var file = path.join(__dirname, "template", "switch.ejs");
      ...
      fs.readFile(file, function(err, data) {
        if (err)
          ...
          conf_script = ejs.render(template, context);
          _callback();
        }
      });
    },
    function(_callback) {
      ...
      fs.writeFile(file, conf_script, function (err) {
        if (err)
          callback(err);
        else
          _callback();
    ...
    logger.debug("Switch " + config["name"] + " successfully configured.");
    callback(null);
  });
}
